...
def simulate(config, actionSeq, actuationInterval=200):
    m = getModel(inFile)
    actions = np.array(actionSeq).reshape(m.numChannels, -1)
    ...
    for action in actionSeq:
        m.inflateChannel = action
        v = m.step(actuationInterval)
        vs.append(v)
    return vs
    
def evalTarget(vs, target, subWeights):
    subTargets = target
    fitness = 0
    for i in range(len(subTargets)):
        fitness += subWeights[i] * subTargets[i](vs)
        
def optimizeAll(config, actionSeqs, targets, weights, subWeightss):
    # optimize actions (targets)
    fitness = 0
    for i in range(len(actionSeqs)):
        v = simulate(config, actionSeqs[i])
        fitness += weights[i] * evalTarget(v, targets[i], subWeightss[i])
    return fitness
